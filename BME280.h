...
#define CS_TH BIT7; //Set CSB to P1.7 on MCU.  Must also be set consistently in main program
...
uint8_t ReadTHid(void){  // Get the TH sensor chip ID: 0x60
    ...
    for (i=0; i < sizeof TID; i++)
    {
        ...
        UCA1TXBUF = TID[i]; //Load data into transmit buffer
        while (!(UCA1IFG & UCRXIFG)); //Wait until complete RX byte is received
        RXbuf[i] = UCA1RXBUF; //Read buffer to clear RX flag
    }
...
void GetCompData(void)
{
    ...
    while (!(UCA1IFG & UCTXIFG)); //Check if it is OK to write
    UCA1TXBUF = 0x88; //Load first address to be read.
    while (!(UCA1IFG & UCRXIFG)); //Wait until complete RX byte is received
    Tempbuf[0] = UCA1RXBUF; //Read buffer to clear RX flag. The first read byte is of no use

    //Burst mode read of temperature, pressure, and first humidity compensation bytes follows
    for (i=1; i < 27; i++)
    {
        while (!(UCA1IFG & UCTXIFG));
        UCA1TXBUF = 0xAA; //Load dummy data into transmit buffer
    ...
    while (!(UCA1IFG & UCTXIFG));
    UCA1TXBUF = 0xE1; //Load first read address
    while (!(UCA1IFG & UCRXIFG));
    Humbuf[0] = UCA1RXBUF; //Read buffer to clear RX flag. This byte is of no use
...
void ReadTHsensor(void)
{
    ...
    {
        while (!(UCA1IFG & UCTXIFG)); //Check if it is OK to write
        UCA1TXBUF = TH1[i]; //Load data into transmit buffer
        while (!(UCA1IFG & UCRXIFG)); //Wait until complete RX byte is received
        RXbuf[i] = UCA1RXBUF; //Read buffer to clear RX flag. These bytes are of no use
    }
    ...
    for (i=0; i < 9; i++) //Burst read.  Address 0x07 auto-increments
    {
        ...
        UCA1TXBUF = 0xAA; //Load dummy data into transmit buffer
        while (!(UCA1IFG & UCRXIFG)); //Wait until complete RX byte is received
        Data[i] = UCA1RXBUF; //Read buffer to clear RX flag
    }
    P1OUT |= CS_TH; //Pull CSB line high
    UCA1CTL1 |= UCSWRST; //Stop USCI
    /*
